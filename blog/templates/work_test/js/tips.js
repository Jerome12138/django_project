...
window.mdSmartios.bridge.pageViewTracking("肌肤检测-注意事项页")
if (window.mdSmartios.bridge.isMeiju && window.mdSmartios.bridge.isAndroid) {
    $("#ios11").css("display","none")
    $(".begin_button").click(()=>{
        window.mdSmartios.bridge.buttonClickTracking("开始拍摄")
        window.mdSmartios.bridge.takePhoto({"compressRage": 10,"type": "png","isNeedBase64": false},res=>{
            // alert(JSON.stringify(res))
            if (res.status == 0) {
                window.mdSmartios.bridge.convertLocalImageToBase64({"filePath":res.filePath},res=>{
                    // alert(JSON.stringify(res))
                    file_img = "data:image/png;base64,"+res.base64
                    $(".user_container").css("display", "none");
                    $(".take_photo").css("display", "block");
                    $(".getPic").attr("src", file_img);
                    $(".getPicImg").css("opacity", "1");
                    document.title = '拍照';
                    $(".if_not").css("display", "flex");
                })
