...
function getRecommendedList() {
    var n = {itemId: cb_entryId, itemTitle: $("#cb_post_title_url").text()};
    $.ajax({
        url: "https://recomm.cnblogs.com/api/v1/recomm/blogpost/reco",
        type: "POST",
        data: JSON.stringify(n),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        ...
        success: function (n) {
            var i, r, t;
            if (n && n.length >= 5) {
                for (i = [], r = '<div class="itnews c_ad_block"><b>相关博文：<\/b><br>', t = 0; t < n.length && t < 5; t++) r += '·  <a href="' + n[t].url.replace("http://", "https://") + '" target="_blank" onclick="clickRecomItmem(' + n[t].item_id + ",'" + n[t].context + "')\">" + n[t].title + "<\/a><br />", i.push({
                    itermId: n[t].item_id,
                    context: n[t].context
                });
                $("#under_post_news").html(r);
                sendRecommView(i)
            }
        }
    })
...
function sendRecommView(n) {
    $.ajax({
        url: "https://recomm.cnblogs.com/api/v1/recomm/blogpost/show-items",
        type: "POST",
        data: JSON.stringify(n),
        contentType: "application/json; charset=utf-8",
        dataType: "json"
...
function clickRecomItmem(n, t) {
    var i = {itemId: n, context: t};
    $.ajax({
        url: "https://recomm.cnblogs.com/api/v1/recomm/blogpost/click-item",
        type: "POST",
